...
export function renderWithProviders(
  ...
  const defaultEventActions = {
    create: () =>
      Promise.resolve({ success: true as const, data: EventFactory.create() }),
    update: () =>
      Promise.resolve({ success: true as const, data: EventFactory.create() }),
    delete: () =>
      Promise.resolve({ success: true as const, data: { deleted: true } }),
    get: () =>
      Promise.resolve({ success: true as const, data: EventFactory.create() }),
  };
  ...
  const defaultScheduleActions = {
    ...
    submit: () =>
      Promise.resolve({
        success: true as const,
        data: { scheduleId: "schedule123" as never },
      }),
      ...
      <I18nextProvider i18n={testI18n}>
        <MantineProvider>
          <NavigationProvider value={defaultNavigation}>
            <ActionsProvider value={defaultActions}>{children}</ActionsProvider>
          </NavigationProvider>
        </MantineProvider>
      </I18nextProvider>
    </QueryClientProvider>
  ...
  // @testing-library/reactのrenderでラップ
