...
import { EventFactory, UserFactory, ScheduleFactory, AvailabilityFactory } from "./factories";
import { Schema } from "effect";
import { EventId, UserId, ScheduleId, NonEmptyString, DateTimeString, EmailString } from "@/lib/effects";
...
describe("EventFactory", () => {
  ...
  it("制限付きのイベントを作成できる", () => {
    const event = EventFactory.withRestrictions("domain", ["example.com", "test.com"]);
...
describe("AvailabilityFactory", () => {
  it("基本的な時間枠を作成できる", () => {
    const availability = AvailabilityFactory.create("2024-03-01T00:00:00.000Z", 600, 630);
  ...
  it("30分スロットを作成できる", () => {
    const availability = AvailabilityFactory.halfHourSlot("2024-03-01T00:00:00.000Z", 14, 30);
  ...
  it("1時間スロットを作成できる", () => {
    const availability = AvailabilityFactory.hourSlot("2024-03-01T00:00:00.000Z", 9);
