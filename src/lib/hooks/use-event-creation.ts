...
export function useEventCreation(locale: string): EventCreationReturn {
  ...
  const updateField = useCallback(<K extends keyof CreateEventFormType>(
    field: K,
    value: CreateEventFormType[K]
  ) => {
  ...
  const handleSubmit = useCallback((e: React.FormEvent) => {
    ...
    const submitPromise = (async () => {
      try {
        ...
        const result = await actions.event.create(submitData);
        ...
        if ("error" in result) {
          ...
          navigation.push(`/${locale satisfies string}/events/${result.data.eventId satisfies string}`);
        }
      } catch {
        setState(prev => ({
          ...prev,
