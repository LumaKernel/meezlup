...
export const UnauthenticatedJapanese: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    // 送信ボタンが表示されていることを確認
    const submitButton = canvas.getByRole("button", { name: /保存|参加可能日時を送信/ });
...
export const UnauthenticatedEnglish: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const nameInput = canvas.queryByRole("textbox", { name: "Name" }) 
      || canvas.getByRole("textbox", { name: /名前/ });
    const emailInput = canvas.queryByRole("textbox", { name: /Email/ }) 
...
export const AuthenticatedUser: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const emailInput = canvas.queryByRole("textbox", { name: /メールアドレス/ });
    await expect(nameInput).toBeInTheDocument();
    await expect(emailInput).toBeInTheDocument();
  },
};
