...
describe("EventCreateFormPresentation", () => {
  const defaultProps = {
    formData: { name: "", description: "", timeSlotDuration: 30 as const, permission: "link-only" as const },
    onFieldChange: vi.fn(),
    onSubmit: vi.fn(),
    onCancel: vi.fn(),
    isSubmitting: false,
  ...
  it("必須フィールドをレンダリングする", () => {
    ...
    expect(screen.getByLabelText("create.timeSlotDuration")).toBeInTheDocument();
  ...
  it("フィールド変更時にonFieldChangeを呼ぶ", async () => {
    ...
    await user.type(screen.getByLabelText("create.eventName"), "新しいイベント");
  ...
  it("送信中はフォームを無効化する", () => {
    ...
    expect(screen.getByRole("button", { name: "create.createEvent" })).toBeDisabled();
  });
  ...
  it("フォームが有効な場合のみ送信ボタンを有効化する", () => {
    ...
    const { rerender } = renderWithProviders(<EventCreateFormPresentation {...invalidProps} />);
    expect(screen.getByRole("button", { name: "create.createEvent" })).toBeDisabled();
    
    rerender(<EventCreateFormPresentation {...validProps} />);
    expect(screen.getByRole("button", { name: "create.createEvent" })).not.toBeDisabled();
  });
});
...
describe("EventCreateFormContainer（統合テスト）", () => {
  it("フォームを正常に送信する", async () => {
    ...
    await user.type(screen.getByLabelText("詳細説明"), "これはテストイベントです");
  ...
  it("エラー時にエラーメッセージを表示する", async () => {
    ...
    expect(screen.queryByText("イベント作成に失敗しました")).not.toBeInTheDocument();
  });
